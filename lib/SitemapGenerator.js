...
function SitemapGenerator(uri, options) {
  ...
  // set user agent
  this.crawler.userAgent = 'OrbitalAds';
  ...
  if(this.options.proxySettings) {
    this.crawler.useProxy= this.options.proxySettings.useProxy || false ;
    if (this.options.proxySettings.useProxy) {
        this.crawler.proxyHostname = this.options.proxySettings.proxyHostname;
        this.crawler.proxyPort = this.options.proxySettings.proxyPort;
    }
  }
  ...
  if(this.options.interval) {
    this.crawler.interval = this.options.interval;
  }
  ...
  this.crawler.on('fetch404', function (queueItem) {
    ...
    if (debug) {
      console.log("Not found [%s]", queueItem.url);
    }
  ...
  this.crawler.on('fetchtimeout', function (queueItem, response) {
    self.store.error.push(queueItem.url);
    if (debug) {
      console.log("Timed out [%s]", queueItem.url);
    }
  ...
  this.crawler.on('fetchdisallowed', function (queueItem) {
    if (self.store.ignored.indexOf(queueItem.url) === -1) {
      ...
      if (debug) {
        console.log("Disallowed [%s]", queueItem.url);
      }
  ...
  this.crawler.on('fetchcomplete', function (queueItem, responseBody,responseObject) {
    self.store.found.push(queueItem.url);
    if (debug) {
      console.log("fetched [%s]", queueItem.url);
    }
